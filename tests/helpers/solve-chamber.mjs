...
export function solveChamber(level) {
  ...
  const orientation=puzzle=>`${puzzle.islands.map(island=>island.quarter).join(',')};${puzzle.seals.map(seal=>Number(seal.unlocked)).join(',')}`;
  ...
  for(let i=0;i<queue.length;i++) {
    ...
    if(at(s.x,s.z).type==='E'&&s.bits===(1<<level.shards.length)-1&&puzzle.seals.every(seal=>seal.unlocked))return s.path;
    ...
    for(const seal of puzzle.seals){
      ...
      if(!bindSeal(bound,seal.id,s,s.mode,seal.yaw,0))continue;
      const next=orientation(bound);
      if(!puzzles.has(next))puzzles.set(next,bound);
      queue.push({...s,orientation:next,path:[...s.path,{type:'bind',sealId:seal.id,yaw:seal.yaw,pitch:0}]});
    }
  }
  return null;
}
