...
A twenty-chamber browser puzzle game. Switch between an overhead view and first person to change which paths are solid. Amber paths belong to the overhead view, blue paths belong to first person, and white stone exists in both. Chambers 1–6 introduce perspective switching; chambers 7–12 add rotating islands, return journeys, and independent hubs. Chambers 13–18 introduce sightline seals; chambers 19–20 combine seals with rotating islands.
...
Choose any of the twenty rooms in **The Journey**, or open `/#chamber-7` to start the island chapter, `/#chamber-13` for sightline seals, or `/#chamber-19` for the combined finale. White hubs remain fixed when their amber arms turn; each island keeps its own orientation. Bound seals keep their bridges open in both perspectives; blue tiles remain walkable only in first person. Restarting restores the chamber's original layout and closes its seals. Saved completions use stable chamber identities, preserving progress from the six-chamber release and the earlier island preview.
...
Scores are shared between the Sites and Vercel demos. Nicknames are public display names, not accounts; an anonymous player identifier on each browser groups its personal bests. A completed score can be retried from the leaderboard if saving fails. The server checks the movement sequence, island turns, seal position and alignment, required fragments, and timing bounds; this is a casual leaderboard, not tamper-proof competitive scoring. Adding the later chapters preserves the existing chambers' leaderboard records.
...
`main` contains the twenty-chamber campaign. Vercel serves `dist/client/`; its leaderboard requests use the shared public Sites endpoint, so no Vercel database or API key is needed. Release the updated Sites backend before publishing the expanded frontend so timed attempts in chambers 13–20 and their seal bindings can be verified.
